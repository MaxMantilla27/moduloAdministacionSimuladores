...
import { DetallePreguntaDTO, LssbEnvioFilePreguntaActualizarDTO, LssbEnvioFilePreguntaDTO, LssbEnvioRespuesDTO} from 'src/app/Models/Lssb/LssbPreguntaDTO';
...
import { LssbModalAlternativasComponent } from './lssb-modal-alternativas/lssb-modal-alternativas.component';

@Component({
  selector: 'app-lssb-modal-agregar-preguntas',
  templateUrl: './lssb-modal-agregar-preguntas.component.html',
  styleUrls: ['./lssb-modal-agregar-preguntas.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class LssbModalAgregarPreguntasComponent implements OnInit {

  constructor(
    @Inject(MAT_DIALOG_DATA) public data: any,
    public dialogRef: MatDialogRef<LssbModalAgregarPreguntasComponent>,
    private _Categorias: LssbCategoriasService,
    private _Tareas: LssbTareaService,
  ...
  public json: LssbEnvioFilePreguntaDTO = {
    Id: 0,
    ...
    IdLssbTipoPreguntaClasificacion: 0,
  ...
  public jsonActualizar: LssbEnvioFilePreguntaActualizarDTO = {
    Id: 0,
    IdSimuladorLssbDominio: 0,
    IdSimuladorLssbTarea: 0,
    IdSimuladorTipoRespuesta: 0,
  ...
  ObtenerDetallePregunta(){
    if (this.data[1] != undefined) {
      ...
      this._pregunta.ObtenerLssbPregunta(this.data[1]).subscribe({
        next: (x: any) => {
          console.log(x)
  ...
  AgregarNuevaPregunta() {
    ...
    this.listaAlternativas.forEach((e: any) => {
      ...
      var alternativas: LssbEnvioRespuesDTO = {
        Id: 0,
  ...
  editarAlternativasTemporal(data: any,index: number) {
    ...
    console.log(data);
    //Editar Pregunta
    const dialogRef = this.dialog.open(LssbModalAlternativasComponent, {
  ...
  FiltrarSubCategoritas() {
    ...
    // if(this.listaCategorias!=undefined){
      this.listaSubCategorias.forEach((ss: any) => {
        if (ss.idSimuladorLssbDominio == idcat) {
  ...
  mostrarMensajeEliminarPreguntaRespuesta(IdRespuesta: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.EliminarPreguntaRespuestaLssb(IdRespuesta);
      }
    });
  }
  ...
  EliminarPreguntaRespuestaLssb(IdRespuesta:number){
    this._alternativa.EliminarPreguntaRespuestaLssb(IdRespuesta).subscribe({
      next: (x: any) => {
      },
      error: (error:any) => {
