...
import { LssbExamenService } from 'src/app/shared/Services/Lssb/Lssb-Examen/lssb-examen.service';
import { LssbAbrirModalDetalleExamenComponent } from './lssb-abrir-modal-detalle-examen/lssb-abrir-modal-detalle-examen.component';

@Component({
  selector: 'app-lssb-reporte-usuarios',
  templateUrl: './lssb-reporte-usuarios.component.html',
  styleUrls: ['./lssb-reporte-usuarios.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class LssbReporteUsuariosComponent implements OnInit {

  datasource=[]
  displayedColumns: string[] = ['ranking', 'nombre', 'fechaInicio', 'FechaFinalizacion', 'duracion', 'puntaje','ver'];

  constructor(
    private _Examen: LssbExamenService,
  ...
  VerExamen(IdExamen:number){
    const dialogRef = this.dialog.open(LssbAbrirModalDetalleExamenComponent, {
      width: '100%',
      height: '100%',
      maxHeight: '95%',
      maxWidth: '100%',
