...
import { DetallePreguntaDTO, CmrpEnvioFilePreguntaActualizarDTO, CmrpEnvioFilePreguntaDTO, CmrpEnvioRespuesDTO} from 'src/app/Models/Cmrp/CmrpPreguntaDTO';
import { AlertaService } from 'src/app/shared/Services/Alerta/alerta.service';
import { CmrpCategoriasService } from 'src/app/shared/Services/Cmrp/Cmrp-Categorias/cmrp-categorias.service';
import { CmrpPreguntaService } from 'src/app/shared/Services/Cmrp/Cmrp-Pregunta/cmrp-pregunta.service';
import { CmrpPreguntaRespuestaService } from 'src/app/shared/Services/Cmrp/Cmrp-Pregunta-Respuesta/cmrp-pregunta-respuesta.service';
import { CmrpTareaService } from 'src/app/shared/Services/Cmrp/Cmrp-Tarea/cmrp-tarea.service';
import Swal from 'sweetalert2';
import { CmrpModalAlternativasComponent } from './cmrp-modal-alternativas/cmrp-modal-alternativas.component';

@Component({
  selector: 'app-cmrp-modal-agregar-preguntas',
  templateUrl: './cmrp-modal-agregar-preguntas.component.html',
  styleUrls: ['./cmrp-modal-agregar-preguntas.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class CmrpModalAgregarPreguntasComponent implements OnInit {

  constructor(
    @Inject(MAT_DIALOG_DATA) public data: any,
    public dialogRef: MatDialogRef<CmrpModalAgregarPreguntasComponent>,
    private _Categorias: CmrpCategoriasService,
    private _Tareas: CmrpTareaService,
    private _alternativa: CmrpPreguntaRespuestaService,
    private _pregunta: CmrpPreguntaService,
  ...
  public json: CmrpEnvioFilePreguntaDTO = {
    Id: 0,
    ...
    IdCmrpTipoPreguntaClasificacion: 0,
  ...
  public jsonActualizar: CmrpEnvioFilePreguntaActualizarDTO = {
    Id: 0,
    IdSimuladorCmrpDominio: 0,
    IdSimuladorCmrpTarea: 0,
    IdSimuladorTipoRespuesta: 0,
  ...
  ObtenerDetallePregunta(){
    if (this.data[1] != undefined) {
      ...
      this._pregunta.ObtenerCmrpPregunta(this.data[1]).subscribe({
        next: (x: any) => {
          console.log(x)
          ...
          this.formPregunta.patchValue({
            Id:x.id,
            IdCategoria:x.idSimuladorCmrpDominio,
            IdSubCategoria:x.idSimuladorCmrpTarea,
  ...
  AgregarNuevaPregunta() {
    ...
    this.json.Id = 0;
    this.json.IdCmrpTipoPreguntaClasificacion = 2;
    this.json.IdSimuladorCmrpDominio = this.formPregunta.get('IdCategoria')?.value;
    this.json.IdSimuladorCmrpTarea = this.formPregunta.get('IdSubCategoria')?.value;
    ...
    this.listaAlternativas.forEach((e: any) => {
      ...
      var alternativas: CmrpEnvioRespuesDTO = {
        Id: 0,
  ...
  ActualizarPregunta(){
    ...
    this.jsonActualizar.IdCmrpTipoPreguntaClasificacion = 2;
    this.jsonActualizar.IdSimuladorCmrpDominio = this.formPregunta.get('IdCategoria')?.value;
    this.jsonActualizar.IdSimuladorCmrpTarea = this.formPregunta.get('IdSubCategoria')?.value;
  ...
  editarAlternativasTemporal(data: any,index: number) {
    ...
    console.log(data);
    //Editar Pregunta
    const dialogRef = this.dialog.open(CmrpModalAlternativasComponent, {
  ...
  FiltrarSubCategoritas() {
    ...
    // if(this.listaCategorias!=undefined){
      this.listaSubCategorias.forEach((ss: any) => {
        if (ss.idSimuladorCmrpDominio == idcat) {
  ...
  mostrarMensajeEliminarPreguntaRespuesta(IdRespuesta: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.EliminarPreguntaRespuestaCmrp(IdRespuesta);
      }
    });
  }
  ...
  EliminarPreguntaRespuestaCmrp(IdRespuesta:number){
    this._alternativa.EliminarPreguntaRespuestaCmrp(IdRespuesta).subscribe({
      next: (x: any) => {
      },
      error: (error:any) => {
