...
import { PbiExamenService } from 'src/app/shared/Services/Pbi/Pbi-examen/pbi-examen.service';
import { PbiTipoRespuestaService } from 'src/app/shared/Services/Pbi/Pbi-Tipo-Respuesta/pbi-tipo-respuesta.service';
import { PbiAbrirModalDetalleExamenComponent } from './pbi-abrir-modal-detalle-examen/pbi-abrir-modal-detalle-examen.component';


@Component({
  selector: 'app-pbi-reporte-usuarios',
  templateUrl: './pbi-reporte-usuarios.component.html',
  styleUrls: ['./pbi-reporte-usuarios.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class PbiReporteUsuariosComponent implements OnInit {

  datasource=[]
  displayedColumns: string[] = ['ranking', 'nombre', 'fechaInicio', 'FechaFinalizacion', 'duracion', 'puntaje','ver'];

  constructor(
    private _Examen: PbiExamenService,
  ...
  VerExamen(IdExamen:number){
    const dialogRef = this.dialog.open(PbiAbrirModalDetalleExamenComponent, {
      width: '100%',
      height: '100%',
      maxHeight: '95%',
      maxWidth: '100%',
