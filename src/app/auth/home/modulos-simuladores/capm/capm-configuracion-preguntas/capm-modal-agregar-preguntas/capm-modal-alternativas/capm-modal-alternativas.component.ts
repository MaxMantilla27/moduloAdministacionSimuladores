...
import { CapmPreguntaRespuestaEnvioAgregarDTO, CapmPreguntaRespuestaEnvioDTO } from 'src/app/Models/Capm/CapmPreguntaRespuestaDTO';
...
export class CapmModalAlternativasComponent implements OnInit {
  ...
  public AgregarAlternativaDTO: CapmPreguntaRespuestaEnvioAgregarDTO = {
    Id:0,
    Respuesta:'',
    Correcto:false,
    Puntaje:0,
  ...
  EnviarRegistrosAlternativas(){
    ...
    if(this.data[3]== undefined ){
      this.AgregarAlternativaDTO.IdSimuladorCapmPregunta = 0
    }
    else{
      this.AgregarAlternativaDTO.IdSimuladorCapmPregunta = this.data[3]
    }


    this._alternativa.AgregarCapmPreguntaRespuesta(this.AgregarAlternativaDTO).subscribe({
      next: (x:any) => {
        console.log(x)
