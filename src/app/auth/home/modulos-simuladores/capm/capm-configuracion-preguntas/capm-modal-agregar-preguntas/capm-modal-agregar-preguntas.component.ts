...
import { DetallePreguntaDTO, CapmEnvioFilePreguntaActualizarDTO, CapmEnvioFilePreguntaDTO, CapmEnvioRespuesDTO} from 'src/app/Models/Capm/CapmPreguntaDTO';
import { AlertaService } from 'src/app/shared/Services/Alerta/alerta.service';
import { CapmCategoriasService } from 'src/app/shared/Services/Capm/Capm-Categorias/capm-categorias.service';
...
import { CapmTareaService } from 'src/app/shared/Services/Capm/Capm-Tarea/capm-tarea.service';
import Swal from 'sweetalert2';
import { CapmModalAlternativasComponent } from './capm-modal-alternativas/capm-modal-alternativas.component';

@Component({
  selector: 'app-capm-modal-agregar-preguntas',
  templateUrl: './capm-modal-agregar-preguntas.component.html',
  styleUrls: ['./capm-modal-agregar-preguntas.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class CapmModalAgregarPreguntasComponent implements OnInit {

  constructor(
    @Inject(MAT_DIALOG_DATA) public data: any,
    public dialogRef: MatDialogRef<CapmModalAgregarPreguntasComponent>,
    private _Categorias: CapmCategoriasService,
    private _Tareas: CapmTareaService,
  ...
  public json: CapmEnvioFilePreguntaDTO = {
    Id: 0,
    ...
    IdCapmTipoPreguntaClasificacion: 0,
  ...
  public jsonActualizar: CapmEnvioFilePreguntaActualizarDTO = {
    Id: 0,
    IdSimuladorCapmDominio: 0,
    IdSimuladorCapmTarea: 0,
    IdSimuladorTipoRespuesta: 0,
      ...
      this._pregunta.ObtenerCapmPregunta(this.data[1]).subscribe({
        next: (x: any) => {
          ...
          this.formPregunta.patchValue({
            Id:x.id,
            IdCategoria:x.idSimuladorCapmDominio,
            IdSubCategoria:x.idSimuladorCapmTarea,
  ...
  AgregarNuevaPregunta() {
    ...
    this.listaAlternativas.forEach((e: any) => {
      ...
      var alternativas: CapmEnvioRespuesDTO = {
        Id: 0,
        IdSimuladorCapmPregunta: 0,
  ...
  editarAlternativasTemporal(data: any,index: number) {
    ...
    console.log(data);
    //Editar Pregunta
    const dialogRef = this.dialog.open(CapmModalAlternativasComponent, {
  ...
  FiltrarSubCategoritas() {
    ...
    // if(this.listaCategorias!=undefined){
      this.listaSubCategorias.forEach((ss: any) => {
        if (ss.idSimuladorCapmDominio == idcat) {
  ...
  mostrarMensajeEliminarPreguntaRespuesta(IdRespuesta: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.EliminarPreguntaRespuestaCapm(IdRespuesta);
      }
    });
  }
  ...
  EliminarPreguntaRespuestaCapm(IdRespuesta:number){
    this._alternativa.EliminarPreguntaRespuestaCapm(IdRespuesta).subscribe({
      next: (x: any) => {
      },
      error: (error:any) => {
