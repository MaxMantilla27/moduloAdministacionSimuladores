...
export class AwsModalAgregarPreguntasComponent implements OnInit {
  ...
  constructor(
    ...
    private _Categorias: AwsCategoriasService,
    private _Tareas: AwsTareaService,
  ...
  public json: AwsEnvioFilePreguntaDTO = {
    Id: 0,
    ...
    IdAwsTipoPreguntaClasificacion: 0,
  ...
  public jsonActualizar: AwsEnvioFilePreguntaActualizarDTO = {
    Id: 0,
    IdSimuladorAwsDominio: 0,
    IdSimuladorAwsTarea: 0,
    IdSimuladorTipoRespuesta: 0,
  ...
  AgregarNuevaPregunta() {
    ...
    this.listaAlternativas.forEach((e: any) => {
      ...
      var alternativas: AwsEnvioRespuesDTO = {
        Id: 0,
  ...
  FiltrarSubCategoritas() {
    ...
    // if(this.listaCategorias!=undefined){
      this.listaSubCategorias.forEach((ss: any) => {
        if (ss.idSimuladorAwsDominio == idcat) {
  ...
  mostrarMensajeEliminarPreguntaRespuesta(IdRespuesta: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.EliminarPreguntaRespuestaAws(IdRespuesta);
      }
    });
  }
  ...
  EliminarPreguntaRespuestaAws(IdRespuesta:number){
    this._alternativa.EliminarPreguntaRespuestaAws(IdRespuesta).subscribe({
      next: (x: any) => {
      },
      error: (error:any) => {
