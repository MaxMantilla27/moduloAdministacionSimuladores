...
import { filtradoPreguntaDTO } from 'src/app/Models/Aws/AwsTipoRespuesta';
import { AlertaService } from 'src/app/shared/Services/Alerta/alerta.service';
import { AwsPreguntaService } from 'src/app/shared/Services/Aws/Aws-Pregunta/aws-pregunta.service';
import Swal from 'sweetalert2';
import { AwsModalAgregarPreguntasComponent } from './aws-modal-agregar-preguntas/aws-modal-agregar-preguntas.component';

@Component({
  selector: 'app-aws-configuracion-preguntas',
  templateUrl: './aws-configuracion-preguntas.component.html',
  styleUrls: ['./aws-configuracion-preguntas.component.scss']
})
export class AwsConfiguracionPreguntasComponent implements OnInit {

  constructor(
    public dialog: MatDialog,
    private _Pregunta: AwsPreguntaService,
  ...
  agregarPregunta() {
    var isNew = true;
    var data=undefined;
    const dialogRef = this.dialog.open(AwsModalAgregarPreguntasComponent, {
  ...
  mostrarMensajeEliminarPregunta(IdPregunta: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.EliminarPreguntaAws(IdPregunta);
      }
    });
  }

  EliminarPreguntaAws(IdPregunta:number){
    this._Pregunta.EliminarPreguntaAws(IdPregunta).subscribe({
      next: (x: any) => {
      },
      error: (error:any) => {
