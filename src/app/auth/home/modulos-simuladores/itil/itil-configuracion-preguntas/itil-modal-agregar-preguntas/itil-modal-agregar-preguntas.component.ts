...
import { DetallePreguntaDTO, ItilEnvioFilePreguntaActualizarDTO, ItilEnvioFilePreguntaDTO, ItilEnvioRespuesDTO} from 'src/app/Models/Itil/ItilPreguntaDTO';
...
import { ItilPreguntaRespuestaService } from 'src/app/shared/Services/Itil/Itil-Pregunta-Respuesta/itil-pregunta-respuesta.service';
import { ItilTareaService } from 'src/app/shared/Services/Itil/Itil-Tarea/itil-tarea.service';
import Swal from 'sweetalert2';
import { ItilModalAlternativasComponent } from './itil-modal-alternativas/itil-modal-alternativas.component';

@Component({
  selector: 'app-itil-modal-agregar-preguntas',
  templateUrl: './itil-modal-agregar-preguntas.component.html',
  styleUrls: ['./itil-modal-agregar-preguntas.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class ItilModalAgregarPreguntasComponent implements OnInit {

  constructor(
    @Inject(MAT_DIALOG_DATA) public data: any,
    public dialogRef: MatDialogRef<ItilModalAgregarPreguntasComponent>,
    private _Categorias: ItilCategoriasService,
    private _Tareas: ItilTareaService,
    private _alternativa: ItilPreguntaRespuestaService,
  ...
  public json: ItilEnvioFilePreguntaDTO = {
    Id: 0,
    ...
    IdItilTipoPreguntaClasificacion: 0,
  ...
  public jsonActualizar: ItilEnvioFilePreguntaActualizarDTO = {
    Id: 0,
    IdSimuladorItilDominio: 0,
    IdSimuladorItilTarea: 0,
    IdSimuladorTipoRespuesta: 0,
  ...
  ObtenerDetallePregunta(){
    if (this.data[1] != undefined) {
      ...
      this._pregunta.ObtenerItilPregunta(this.data[1]).subscribe({
        next: (x: any) => {
          console.log(x)
          ...
          this.formPregunta.patchValue({
            Id:x.id,
            IdCategoria:x.idSimuladorItilDominio,
            IdSubCategoria:x.idSimuladorItilTarea,
  ...
  AgregarNuevaPregunta() {
    ...
    this.listaAlternativas.forEach((e: any) => {
      ...
      var alternativas: ItilEnvioRespuesDTO = {
        Id: 0,
  ...
  editarAlternativasTemporal(data: any,index: number) {
    ...
    console.log(data);
    //Editar Pregunta
    const dialogRef = this.dialog.open(ItilModalAlternativasComponent, {
  ...
  FiltrarSubCategoritas() {
    ...
    // if(this.listaCategorias!=undefined){
      this.listaSubCategorias.forEach((ss: any) => {
        if (ss.idSimuladorItilDominio == idcat) {
  ...
  mostrarMensajeEliminarPreguntaRespuesta(IdRespuesta: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.EliminarPreguntaRespuestaItil(IdRespuesta);
      }
    });
  }
  ...
  EliminarPreguntaRespuestaItil(IdRespuesta:number){
    this._alternativa.EliminarPreguntaRespuestaItil(IdRespuesta).subscribe({
      next: (x: any) => {
      },
      error: (error:any) => {
