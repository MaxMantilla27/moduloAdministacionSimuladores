...
import { DetallePreguntaDTO, PacpEnvioFilePreguntaActualizarDTO, PacpEnvioFilePreguntaDTO, PacpEnvioRespuesDTO } from 'src/app/Models/Pacp/PacpPreguntaDTO';
...
import { PacpPreguntaRespuestaService } from 'src/app/shared/Services/Pacp/Pacp-PreguntaRespuesta/pacp-pregunta-respuesta.service';
import { PacpTareaService } from 'src/app/shared/Services/Pacp/Pacp-Tarea/pacp-tarea.service';
import Swal from 'sweetalert2';
import { PacpModalAlternativasComponent } from './pacp-modal-alternativas/pacp-modal-alternativas.component';

@Component({
  selector: 'app-pacp-modal-agregar-preguntas',
  templateUrl: './pacp-modal-agregar-preguntas.component.html',
  styleUrls: ['./pacp-modal-agregar-preguntas.component.scss']
})
export class PacpModalAgregarPreguntasComponent implements OnInit {

  constructor(
    @Inject(MAT_DIALOG_DATA) public data: any,
    public dialogRef: MatDialogRef<PacpModalAgregarPreguntasComponent>,
    private _Categorias: PacpCategoriasService,
    private _Tareas: PacpTareaService,
    private _alternativa: PacpPreguntaRespuestaService,
  ...
  public json: PacpEnvioFilePreguntaDTO = {
    Id: 0,
    ...
    IdPacpTipoPreguntaClasificacion: 0,
  ...
  public jsonActualizar: PacpEnvioFilePreguntaActualizarDTO = {
    Id: 0,
    IdSimuladorPacpDominio: 0,
    IdSimuladorPacpTarea: 0,
    IdSimuladorTipoRespuesta: 0,
  ...
  ObtenerDetallePregunta(){
    if (this.data[1] != undefined) {
      ...
      this._pregunta.ObtenerPacpPregunta(this.data[1].id).subscribe({
        next: (x: any) => {
          console.log(x)
          ...
          this.formPregunta.patchValue({
            Id:x[0].id,
            IdCategoria:x[0].idSimuladorPacpDominio,
            IdSubCategoria:x[0].idSimuladorPacpTarea,
  ...
  AgregarNuevaPregunta() {
    ...
    this.json.Id = 0;
    this.json.IdPacpTipoPreguntaClasificacion = 2;
    this.json.IdSimuladorPacpDominio = this.formPregunta.get('IdCategoria')?.value;
    this.json.IdSimuladorPacpTarea = this.formPregunta.get('IdCategoria')?.value;
    ...
    this.listaAlternativas.forEach((e: any) => {
      ...
      var alternativas: PacpEnvioRespuesDTO = {
        Id: 0,
  ...
  ActualizarPregunta(){
    ...
    this.jsonActualizar.IdSimuladorPacpTarea = this.formPregunta.get('IdCategoria')?.value;
  ...
  editarAlternativasTemporal(data: any,index: number) {
    ...
    console.log(data);
    //Editar Pregunta
    const dialogRef = this.dialog.open(PacpModalAlternativasComponent, {
  ...
  FiltrarSubCategoritas() {
    ...
    // if(this.listaCategorias!=undefined){
      this.listaSubCategorias.forEach((ss: any) => {
        if (ss.idSimuladorPacpDominio == idcat) {
  ...
  mostrarMensajeEliminarPreguntaRespuesta(IdRespuesta: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.EliminarPreguntaRespuestaPacp(IdRespuesta);
      }
    });
  }
  ...
  EliminarPreguntaRespuestaPacp(IdRespuesta:number){
    this._alternativa.EliminarPreguntaRespuestaPacp(IdRespuesta).subscribe({
      next: (x: any) => {
      },
      error: (error:any) => {
