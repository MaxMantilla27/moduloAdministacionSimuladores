...
import { filtradoPreguntaDTO } from 'src/app/Models/Adsa/AdsaTipoRespuesta';
...
import { AdsaModalAgregarPreguntasComponent } from './adsa-modal-agregar-preguntas/adsa-modal-agregar-preguntas.component';

@Component({
  selector: 'app-adsa-configuracion-preguntas',
  templateUrl: './adsa-configuracion-preguntas.component.html',
  styleUrls: ['./adsa-configuracion-preguntas.component.scss'],
})
export class AdsaConfiguracionPreguntasComponent implements OnInit {
  constructor(
  ...
  ngOnInit(): void {
    this.ObtenerPreguntasAdsa();
  }

  ObtenerPreguntasAdsa() {
  ...
  agregarPregunta() {
    ...
    const dialogRef = this.dialog.open(AdsaModalAgregarPreguntasComponent, {
  ...
  mostrarMensajeEliminarPregunta(IdPregunta: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        console.log(IdPregunta)
        this.EliminarPreguntaAdsa(IdPregunta);
      }
    });
  }

  EliminarPreguntaAdsa(IdPregunta:number){
    this._Pregunta.EliminarPreguntaAdsa(IdPregunta).subscribe({
      next: (x: any) => {
      },
      error: (error:any) => {
