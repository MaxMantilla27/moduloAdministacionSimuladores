...
import { AdsaExamenService } from 'src/app/shared/Services/Adsa/Adsa-examen/adsa-examen.service';
import { AdsaTipoRespuestaService } from 'src/app/shared/Services/Adsa/Adsa-Tipo-Respuesta/adsa-tipo-respuesta.service';
import { AdsaAbrirModalDetalleExamenComponent } from './adsa-abrir-modal-detalle-examen/adsa-abrir-modal-detalle-examen.component';


@Component({
  selector: 'app-adsa-reporte-usuarios',
  templateUrl: './adsa-reporte-usuarios.component.html',
  styleUrls: ['./adsa-reporte-usuarios.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class AdsaReporteUsuariosComponent implements OnInit {

  datasource=[]
  displayedColumns: string[] = ['ranking', 'nombre', 'fechaInicio', 'FechaFinalizacion', 'duracion', 'puntaje','ver'];

  constructor(
    private _Examen: AdsaExamenService,
  ...
  VerExamen(IdExamen:number){
    const dialogRef = this.dialog.open(AdsaAbrirModalDetalleExamenComponent, {
      width: '100%',
      height: '100%',
      maxHeight: '95%',
      maxWidth: '100%',
